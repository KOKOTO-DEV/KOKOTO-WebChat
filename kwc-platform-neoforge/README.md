...
KWC 5.2.0 NeoForge is built as 12 exact targets from Minecraft 1.20.2 through 26.2.
...
Deploy only the JAR whose Minecraft suffix exactly matches the server version. For example, a Minecraft 1.21.1 NeoForge server must use `KOKOTO-WebChat-5.2.0-NeoForge-1.21.1.jar`; a `NeoForge-26.2.jar` is not interchangeable with it.
