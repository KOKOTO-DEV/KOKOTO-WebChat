 ...
 * NeoForgeStorage는 NeoForge 런타임에서 KWC core 기능을 해당 loader/Minecraft API에 연결한다.
 * NeoForgeStorage connects KWC core behavior to the concrete NeoForge/Minecraft runtime APIs.
 *
...
public class NeoForgeStorage implements WebChatStorage {
    ...
    public NeoForgeStorage(KwcNeoForgeRuntime plugin) {
        this.plugin = plugin;
    ...
    public synchronized void load() {
        ...
        NeoForgeYamlConfiguration accounts = NeoForgeYamlConfiguration.loadConfiguration(accountsFile);
        NeoForgeYamlSection root = accounts.getConfigurationSection("accounts");
        if (root != null) {
            for (String id : root.getKeys(false)) {
                NeoForgeYamlSection s = root.getConfigurationSection(id);
                if (s == null) continue;
        ...
        NeoForgeYamlConfiguration sessions = NeoForgeYamlConfiguration.loadConfiguration(sessionsFile);
        NeoForgeYamlSection sr = sessions.getConfigurationSection("sessions");
        if (sr != null) {
            for (String hash : sr.getKeys(false)) {
                NeoForgeYamlSection s = sr.getConfigurationSection(hash);
                if (s == null) continue;
    ...
    private synchronized void loadKnownDisplayNames() {
        knownDisplayNamesByUuid.clear();
        knownUsernamesByUuid.clear();
        NeoForgeYamlConfiguration yml = NeoForgeYamlConfiguration.loadConfiguration(knownDisplayNamesFile);
        NeoForgeYamlSection root = yml.getConfigurationSection("players");
        if (root == null) return;
        for (String uuid : root.getKeys(false)) {
            NeoForgeYamlSection s = root.getConfigurationSection(uuid);
            if (s == null) continue;
    ...
    public synchronized void loadPinnedMessages() {
        pinnedById.clear();
        NeoForgeYamlConfiguration yml = NeoForgeYamlConfiguration.loadConfiguration(pinnedFile);
        NeoForgeYamlSection root = yml.getConfigurationSection("pinned");
        if (root == null) return;
        for (String id : root.getKeys(false)) {
            NeoForgeYamlSection s = root.getConfigurationSection(id);
    ...
    private static PinnedMessage pinnedMessageFromSection(NeoForgeYamlSection s) {
