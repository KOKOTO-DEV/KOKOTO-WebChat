 ...
 * NeoForgeConfigValuesLoader는 KWC 설정을 core가 사용할 수 있는 형태로 읽거나 보관하는 설정 계층이다.
 * NeoForgeConfigValuesLoader is part of the configuration layer that reads or carries KWC settings in a core-friendly form.
 *
...
/** NeoForge YAML loader for the platform-neutral ConfigValues data object. */
public final class NeoForgeConfigValuesLoader {
    private NeoForgeConfigValuesLoader() {}

    public static ConfigValues load(NeoForgeYamlConfiguration c) {
        ...
        NeoForgeYamlSection tokenCustom = c.getConfigurationSection("message-tokens.custom");
        if (tokenCustom != null) {
            for (String key : tokenCustom.getKeys(false)) {
                NeoForgeYamlSection item = tokenCustom.getConfigurationSection(key);
        ...
        NeoForgeYamlSection gameLinkAliases = c.getConfigurationSection("emoji.game-link.aliases");
    ...
    private static void loadAnnouncement(ConfigValues v, NeoForgeYamlConfiguration c, String key, boolean defaultEnabled, String defaultMessage) {
    ...
    private static boolean configBool(NeoForgeYamlConfiguration c, String path, boolean def) {
        return c != null && c.contains(path) ? c.getBoolean(path) : def;
    }

    private static boolean notificationBool(NeoForgeYamlConfiguration c, String key, boolean def) {
        return configBool(c, "notifications." + key, def);
    }

    private static boolean legacyPairBool(NeoForgeYamlConfiguration c, String browserPath, String webPushPath, boolean def) {
    ...
    private static String translateConfiguredGameFormat(String value) {
        return translateAlternateColorCodes(String.valueOf(value == null ? "" : value));
    }
