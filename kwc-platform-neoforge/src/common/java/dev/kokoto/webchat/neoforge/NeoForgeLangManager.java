 ...
 * NeoForgeLangManager는 여러 저수준 객체를 조합해 하나의 KWC 기능 흐름을 수행하는 서비스/관리 계층이다.
 * NeoForgeLangManager is a service/manager layer coordinating lower-level objects into one KWC feature flow.
 *
...
public class NeoForgeLangManager implements WebChatLanguage {
    ...
    public NeoForgeLangManager(KwcNeoForgeRuntime plugin) {
        this.plugin = plugin;
    }
    ...
    private void updateBuiltinLanguageFile(File target, String resource) {
        try (InputStream in = plugin.resource(resource)) {
            ...
            NeoForgeYamlConfiguration existing = NeoForgeYamlConfiguration.loadConfiguration(target);
            NeoForgeYamlConfiguration builtin = NeoForgeYamlConfiguration.loadConfiguration(new java.io.InputStreamReader(in, java.nio.charset.StandardCharsets.UTF_8));
    ...
    private boolean migrateChangedBuiltinStrings(String language, NeoForgeYamlConfiguration existing, NeoForgeYamlConfiguration builtin) {
    ...
    private boolean migrateLegacyBranding(NeoForgeYamlConfiguration existing, NeoForgeYamlConfiguration builtin) {
        boolean changed = false;
        String[][] legacyValues = {
    ...
    private Map<String, String> defaultKeys() {
        ...
        try (InputStream in = plugin.resource("lang/en-US.yml")) {
            if (in == null) return m;
            NeoForgeYamlConfiguration builtin = NeoForgeYamlConfiguration.loadConfiguration(new java.io.InputStreamReader(in, java.nio.charset.StandardCharsets.UTF_8));
            NeoForgeYamlSection web = builtin.getConfigurationSection("web");
