...
/** Fabric YAML loader for the platform-neutral ConfigValues data object. */
public final class FabricConfigValuesLoader {
    private FabricConfigValuesLoader() {}

    public static ConfigValues load(FabricYamlConfiguration c) {
        ...
        FabricYamlSection tokenCustom = c.getConfigurationSection("message-tokens.custom");
        if (tokenCustom != null) {
            for (String key : tokenCustom.getKeys(false)) {
                FabricYamlSection item = tokenCustom.getConfigurationSection(key);
        ...
        FabricYamlSection gameLinkAliases = c.getConfigurationSection("emoji.game-link.aliases");
    ...
    private static void loadAnnouncement(ConfigValues v, FabricYamlConfiguration c, String key, boolean defaultEnabled, String defaultMessage) {
    ...
    private static boolean configBool(FabricYamlConfiguration c, String path, boolean def) {
        return c != null && c.contains(path) ? c.getBoolean(path) : def;
    }

    private static boolean notificationBool(FabricYamlConfiguration c, String key, boolean def) {
        return configBool(c, "notifications." + key, def);
    }

    private static boolean legacyPairBool(FabricYamlConfiguration c, String browserPath, String webPushPath, boolean def) {
