 ...
 * FabricModerationManager는 메시지 moderation/content-filter 파이프라인의 일부다.
 * FabricModerationManager is part of the message moderation/content-filter pipeline.
 *
...
public class FabricModerationManager implements WebChatModeration {
    private final KwcFabricRuntime plugin;
    private final File file;
    private final Map<String, ModerationEntry> entries = new ConcurrentHashMap<>();

    public FabricModerationManager(KwcFabricRuntime plugin) {
        this.plugin = plugin;
    ...
    public synchronized void load() {
        ...
        FabricYamlSection root = yml.getConfigurationSection("entries");
        if (root == null) return;
        for (String key : root.getKeys(false)) {
            FabricYamlSection s = root.getConfigurationSection(key);
            if (s == null) continue;
