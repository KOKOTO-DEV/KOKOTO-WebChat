 ...
 * FabricYamlSection는 Fabric 런타임에서 KWC core 기능을 해당 loader/Minecraft API에 연결한다.
 * FabricYamlSection connects KWC core behavior to the concrete Fabric/Minecraft runtime APIs.
 *
...
public class FabricYamlSection {
    protected final Map<String, Object> root;
    protected final Map<String, Object> section;
    protected final String sectionName;

    FabricYamlSection(Map<String, Object> root, Map<String, Object> section, String sectionName) {
        this.root = root;
    ...
    @SuppressWarnings("unchecked")
    public FabricYamlSection getConfigurationSection(String path) {
        Object v = get(path);
        if (!(v instanceof Map<?, ?> map)) return null;
        String name = path;
        int dot = name.lastIndexOf('.');
        if (dot >= 0) name = name.substring(dot + 1);
        return new FabricYamlSection(root, (Map<String,Object>) map, name);
    }
