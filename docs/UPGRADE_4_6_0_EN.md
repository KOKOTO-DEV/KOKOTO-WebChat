...
Stop the server and back up `plugins/BlueMapWebChat`, especially `config.yml`, SQLite databases and their `-wal`/`-shm` files, DM/group databases, uploads, emojis, custom language files, audit logs, and Web Push key/subscription files.

## Generated configuration migration fragment

BlueMapWebChat never overwrites or automatically merges an existing `config.yml`. On server start and `/bmchat reload`, it reads the physical file and checks `config-version`.
...
3. Add `config-version: "4.6.0"`, run `/bmchat reload`, and confirm that comparison is skipped.
