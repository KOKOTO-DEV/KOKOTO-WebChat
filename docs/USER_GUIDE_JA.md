...
このガイドでは日常的な利用方法を説明します。サーバーの導入・保守は `INSTALLATION_OPERATIONS_JA.md`、実装の詳細は `TECHNICAL_REFERENCE_JA.md`、長文の総合リファレンスは `COMPLETE_REFERENCE_JA.md` を参照してください。
...
実装/セキュリティの詳細と外部標準については [TECHNICAL_REFERENCE_JA.md](TECHNICAL_REFERENCE_JA.md) と [REFERENCES_JA.md](REFERENCES_JA.md) を参照してください。
