...
## 配置生命周期
编辑当前 `config.yml` 后执行 `/kchat reload`。Reload 会在替换正在运行的服务之前验证 YAML；如果 YAML 无效，原有运行配置会继续保持。`config-reference-5.1.0.yml` 是面向管理员的当前默认配置参考文件，会使用与受支持内置 `ui.language` 相同的语言生成；自定义或不受支持的 UI 语言使用英文展示。
