# KOKOTO WebChat 5.2.0 Release Checklist

## Source/config/i18n
- [ ] Root/Bukkit/Fabric/NeoForge/Forge metadata and artifact names are `5.2.0`.
- [ ] `config.yml`, `config-baselines/config-5.2.0.yml`, and `distribution/config-reference-5.2.0.yml` are byte-identical.
- [ ] 5.1.0 → 5.2.0 migration writes `5.2.0_auto_migration`, preserves supported operator values including existing Relay v2 groups/secrets/peers, removes retired settings, and exact `5.2.0` stops same-version reconstruction. Pre-5.1.0 migration still resets legacy Relay v1 trust/topology for explicit Relay v2 reconfiguration.
