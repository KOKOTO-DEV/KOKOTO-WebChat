...
- [ ] Public/DM/group latest-follow runs only while the remaining gap is **less than two rendered text lines**; readers farther up are not forced to bottom by new messages/compose-panel layout changes, and refresh/reconnect restores the saved conversation position when possible.
