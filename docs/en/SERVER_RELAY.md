...
This relay-specific security change applies to deployments that actually enabled/configured **Relay Protocol v1** in KWC 5.0.0 or a compatible BMWC relay peer. Servers that did not use server relay are not exposed to this relay transport/trust weakness.

- **Highest priority:** any Relay v1 peer configured with `http://`. Relay v1 payloads were plaintext on that path when TLS was not present.
- Relay v1 over HTTPS avoided passive plaintext reading on the network hop, but it still used one flat peer trust set and a top-level shared secret. Secret exposure or an unintended peer/forwarding configuration therefore had a wider trust impact than the explicit group boundaries used by v2.
- This is a protocol-design hardening statement, not a claim that a specific installation was exploited or that a CVE has been assigned.
