...
- **Auto-follow and view restoration:** public, DM, and group chat use the currently rendered text line height. KWC follows the newest messages only while the remaining bottom gap is **less than two text lines**; readers farther up are not pulled down merely by a new message or an emoji/icon/attachment layout change. After refresh/reconnect, KWC restores the saved conversation position when possible.
...
In **Standalone desktop multi-window mode**, each detached DM or group conversation window is also a direct drag-and-drop target. Dropping files on a child window activates that exact thread/room and inserts the uploaded URLs into that conversation; the parent DM/group list window remains a valid drop target as well. Embedded map-adapter presentation is not documented as a supported multi-window deployment mode.
