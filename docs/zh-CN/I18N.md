...
`ui.language` 也用于 KOKOTO WebChat 重建 `config.yml` 注释/布局、生成 `config-reference-5.2.0.yml` 以及 migration/Difference 提示文本时的显示语言。内置配置显示模板为 `en-US`、`ko-KR`、`ja-JP`、`zh-CN`。程序会把现有已解析的实际配置值覆盖到所选模板上，因此在这四种语言之间切换时只会改变注释/布局，实际配置值会保留。
