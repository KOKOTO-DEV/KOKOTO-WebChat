...
公共、DM、群聊统一使用 32px 最新消息自动跟随阈值。打开/关闭 emoji/icon/attachment 面板造成的布局变化会保留当前 viewport，不会仅因该变化强制滚到底部。
