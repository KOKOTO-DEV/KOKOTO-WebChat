...
BlueMapWebChat 4.6.0 includes an optional compatibility path for [ImageEmojis-Bero 1.9.0](https://github.com/KOKOTO-DEV/ImageEmojis-Bero). The integration is reflection-based and does not add a hard dependency, so BlueMapWebChat still starts when ImageEmojis-Bero is not installed.
