...
릴리스 번호는 4.6.1로 유지합니다. 서버 간 DM을 주고받는 모든 서버에 수정된 4.6.1 빌드를 설치해야 합니다. `서버명 · 종류` 클릭 시 해당 글의 UUID와 원본 서버 ID를 직접 전달하며, 타 서버 검색 결과와 기존 원격 대화에서도 대상 서버 ID와 플레이어 UUID를 함께 유지합니다.
