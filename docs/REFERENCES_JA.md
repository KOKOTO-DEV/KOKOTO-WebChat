# KOKOTO WebChat 5.1.0 — 参照規格と公式ドキュメント

このページでは、KOKOTO WebChat 5.1.0 の技術・運用ドキュメントが参照する**一次規格と公式製品／プロジェクト文書**をまとめます。KWC 固有の動作は、このリポジトリの 5.1.0 ソースコードと現行リリース文書が基準です。以下のリンクは、基盤となるプロトコル、API、保存方式、外部連携を説明するための参照資料です。
...
上流 文書は元プロジェクトの動作を説明するものです。KWC-Bero 連携や KWC 固有の互換性は、該当する KWC 文書と 5.1.0 ソースコードを基準にしてください。
...
- KWC の実動作、既定値、互換性ルール、セキュリティ境界は、必ず 5.1.0 ソースコードと現行 KWC 文書で確認します。
- 過去バージョンの文書は履歴資料として保持し、現在の 5.1.0 動作定義には使用しません。
