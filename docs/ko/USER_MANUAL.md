...
| 영역 | 도식 |
| --- | --- |
| 아키텍처 | [PNG](../assets/architecture-5.3.0.png) · [SVG](../assets/architecture-5.3.0.svg) |
| Relay Protocol v2 | [Animated GIF](../assets/relay-v2-flow.gif) · [PNG](../assets/relay-v2-flow.png) · [SVG](../assets/relay-v2-flow.svg) |
| DM/그룹 Reply | [PNG](../assets/private-reply-flow.png) · [SVG](../assets/private-reply-flow.svg) |
| 설정 migration | [Animated GIF](../assets/config-language-migration.gif) · [PNG](../assets/config-language-migration.png) · [SVG](../assets/config-language-migration.svg) |
| 배포 모드 | [PNG](../assets/deployment-modes.png) · [SVG](../assets/deployment-modes.svg) |
| 업로드 보안 | [PNG](../assets/upload-security-pipeline.png) · [SVG](../assets/upload-security-pipeline.svg) |
...
- **Relay 2.2:** protocol major 2 안에서 발신자 소유 DM 삭제, Chat Event 라우팅, 원격 공개 프로필 조회를 capability로 제어합니다. 각 peer는 `public-chat`, `event`, `dm`, `profile`의 송신/수신을 독립적으로 허용할 수 있습니다.
- **창/UI와 어댑터:** 공개채팅 창의 이동·크기조절·최소화 동작은 맵 어댑터와 Standalone에서 동기화하며, 모바일/작은 화면의 임베드 어댑터에서도 최소화 버튼을 유지합니다. 여러 DM/그룹 대화를 독립 창으로 여는 멀티윈도우는 충분히 큰 **Standalone 데스크톱** 화면에서만 지원합니다. 모든 wrapper는 같은 frontend fragment와 CSS에서 생성합니다.
- **이미지 개인정보:** 업로드/프로필 이미지의 지원 가능한 EXIF/IPTC/comment/XMP 메타데이터를 저장 전에 제거합니다.

## 5.2.0 추가 기능

- **메시지 반응:** 로그인 사용자는 공개채팅, DM, 일반 그룹채팅 메시지에 Unicode 또는 KWC 커스텀 이모지 반응을 추가/해제할 수 있습니다. 그룹 입장/퇴장 이벤트는 제외됩니다. 실제 반응이 없을 때 32 × 16px `+` 버튼은 본문 아래와 다음 메시지 앞에 각각 1px의 시각적 여유를 두어 텍스트를 덮지 않으며, 반응 OFF에서는 빈 요소를 만들지 않아 기존 8px 메시지 간격을 그대로 사용합니다. 실제 반응이 생긴 뒤에만 정상 in-flow 반응 행/칩 간격을 사용합니다. picker는 이모지 문자 자체, 서버 생성 Unicode 이름, 관리자 검색 별칭, 커스텀 이모지 ID/이름/팩을 검색합니다. 검색 별칭은 **관리자 > 이모지 > 반응 아이콘**에서 `이모지 = 검색어` 형식으로 편집하고 `reaction-search-aliases.txt`에 저장합니다. 검색 별칭은 picker 검색 전용이며 공개채팅/DM/그룹채팅 입력을 변환하지 않습니다. 카테고리/검색 재렌더 뒤에도 원래 위치와 바깥 클릭 닫기를 유지하며, chip hover에서는 Minecraft 서식을 제거한 반응자 이름 전체를 최대 4줄 높이의 스크롤 목록으로 보여주며, 일반 채팅 송신자와 같은 표시이름 ↔ 원래이름 전환을 사용합니다. 같은 관리자 화면의 전체 ON/OFF 및 KWC 커스텀 이모지 허용은 다른 Admin 설정과 동일한 둥근 테마 행을 사용합니다. 기능을 끄면 기존 반응 데이터는 보존되고 칩은 읽기 전용으로 표시되며 모든 로컬/Relay 반응 변경은 거부됩니다. Relay 2.1은 Relay 공개 메시지와 타 서버 DM의 반응 및 reactor 표시이름을 동기화하되 actor UUID는 서버/relay 내부에만 유지하며 메시지/미디어 DOM 전체를 다시 만들지 않습니다.
- **대화 저장:** `chat.conversation-archive.enabled: true`(기본값)일 때 공개/DM/그룹 대화의 첫 메시지와 끝 메시지를 지정해 개인 snapshot으로 저장합니다. 서버가 끄면 저장 관련 DOM을 생성하지 않고 archive API도 등록하지 않으며 archive DB도 열거나 새로 만들지 않습니다. 일반 retention으로 원본이 사라져도 snapshot은 유지하지만 관리자 강제 원본 삭제/방 삭제와 비공개 방 잠금 정책은 항상 우선합니다. 첨부 바이트는 복제하지 않으며, 원본 이미지가 남아 있으면 인쇄/PDF에 표시하고 다른 파일은 링크로 유지하며 원본이 없으면 유실로 표시합니다. PDF는 현재 사용자의 KWC 표시 설정을 사용합니다. 서버의 저장 한도는 `chat.conversation-archive` 아래 `max-archives-per-user`, `max-messages-per-archive`, `max-messages-per-user`로 제한합니다.
- **`입력 중...`:** 공개/DM/그룹 typing은 이벤트 기반입니다. 서버 관리자가 Web Admin **Settings** 또는 `chat.typing-indicator.*`에서 오픈채팅/DM/그룹채팅을 각각 켜거나 끄며 기본값은 OFF/ON/ON입니다. `chat.typing-indicator.user-display-control`은 기본 OFF이며, 관리자가 켜면 로그인 사용자의 채팅 설정에 계정 저장형 **입력 중 표시** 하나가 나타납니다. 개인 OFF는 본인 화면의 수신 표시만 숨기고 본인의 typing 전송에는 영향을 주지 않습니다. 첫 입력 이벤트로 5초 표시하고 그 기간에는 추가 전송하지 않으며 polling, 메시지 DB 기록, 상시 typing worker가 없습니다. 표시는 입력창 한 줄 위에 둥근 고선명 pill로 떠 있고 일반 채팅과 같은 표시이름 ↔ 원래이름 전환을 사용하며 이름의 formatting tag는 제거합니다. 글꼴은 사용자 채팅 글꼴의 약 80%를 따르되 서버 기본 UI 글꼴보다 작아지지 않습니다. 그룹의 여러 긴 이름이 한 줄에 들어가지 않으면 인원수 표시로 자동 축약합니다.
- **비공개 방 제목창:** 긴 제목만 `...` 처리하고 접속인원·설정·나가기는 폭을 유지합니다. 초대, 대화 저장, 숨기기, 방 관리 기능은 권한에 따라 설정 메뉴에 모읍니다.
- **자동 최하단 추종 및 위치 복원:** 공개/DM/그룹 모두 현재 실제로 렌더링되는 line-height를 기준으로 하며, 최하단까지 남은 간격이 **2줄 미만**일 때만 최신 메시지를 자동으로 따라갑니다. 그보다 위를 보고 있으면 새 메시지나 이모지/아이콘/첨부 패널의 레이아웃 변화만으로 최하단으로 끌어내리지 않습니다. 새로고침/재연결 후에도 가능한 경우 저장된 원래 대화 위치를 복원합니다.
- **Relay 2.1:** KWC 제품 버전과 relay protocol revision을 분리했습니다. Protocol major 2가 wire compatibility 경계이며 2.1은 `public`, `dm`, `read`, `reaction`, `reaction-authority`, `typing` capability를 알립니다.
...
- 실제 파일의 표식을 `config-version: "5.3.0_auto_migration"`로 바꿉니다.
...
| 실제 `config.yml` 상태 | 동작 |
|---|---|
| `config-version` 없음 또는 이전/다른 버전 | migration을 수행하고 `5.3.0_auto_migration`으로 표시한 뒤 migration/검토 보고서 생성 |
| `config-version: "5.3.0_auto_migration"` | 자동 migration 사용. startup/reload마다 최신 같은 버전 번들 `config.yml`을 새 뼈대로 만들고 현재 값을 덮어쓴 뒤 migration report/diff 갱신 |
| `config-version: "5.3.0"` | 현재 버전의 자동 migration 중지. 같은 버전 migration/backfill을 건너뛰고 오래된 migration 안내 제거 |
...
5.3.0의 **사용자 프로필 카드**는 공개채팅/DM/그룹/사용자 목록의 이름을 누르거나 계정/사용자 메뉴에서 내 프로필을 열어 사용합니다. 채팅 설정에 접속상태/차단 관리가 중복 노출되지 않으며, 본인 프로필에서 온라인 상태(온라인/바쁨/오프라인)와 차단 사용자 목록을 관리합니다. 프로필에는 표시이름↔실제이름 토글, Game/Web 접속 상태, 다른 사용자에게 DM 보내기, 280자 소개/한마디가 있습니다. 다른 사용자의 Game/Web 상태 표시는 직접 DM을 여는 바로가기이며 사용자 이름은 계속 프로필을 엽니다. ADMIN은 다른 로컬 계정 프로필에서 USER/MODERATOR/ADMIN 역할을 변경할 수 있고 역할은 메시지 접두사 대신 프로필에 표시됩니다. 프로필 이미지는 없음/Minecraft Head/사용자 이미지 중 선택할 수 있고, 사용자 이미지는 PNG/JPEG/WebP 최대 2 MiB이며 EXIF 등 이미지 메타데이터를 제거한 뒤 일반 채팅 업로드와 분리된 계정 전용 저장소에 보관합니다. **오프라인**은 다른 사용자에게 실제 Game/Web 상태를 숨기고 온라인 인원/기본 온라인 목록에서도 제외합니다. 관리자 온라인 목록에서는 `오프라인 사용자 표시`를 선택해 알려진 오프라인 계정을 함께 볼 수 있지만 실제 미접속과 오프라인 표시 사용자를 구분하지 않습니다.
...
KOKOTO WebChat은 메시지를 저장하거나 릴레이하기 전에 관리자가 설정한 `:alias:` 토큰을 치환할 수 있습니다. 기본 alias는 영어만 제공하며 관리자가 원하는 언어의 alias로 바꾸거나 추가할 수 있습니다.
...
`captcha.mode`는 `off` / `math` / `text` / `mixed`를 지원합니다. 산술 문제는 `Solve:` 같은 언어 문구 없이 `7 + 5 = ?` 형태의 식만 표시합니다. `captcha.math-complexity`는 `easy`(한 자리 덧셈/뺄셈), `normal`(덧셈/뺄셈/곱셈), `hard`(더 큰 수와 정수로 나누어떨어지는 나눗셈 포함)를 선택합니다. 문자형의 `코드 입력:` 안내는 KWC 선택 언어로 번역됩니다.
...
**Standalone 데스크톱 모드**에서 작업영역이 **900 × 480 이상**이면 메시지함은 대화 목록 부모 창으로 남고, 각 DM을 독립 자식 창으로 열 수 있습니다. 같은 DM을 다시 선택하면 새 창을 만들지 않고 기존 창을 활성화합니다. 자식창은 제목 드래그 이동, 바깥쪽 투명 8방향 resize 영역, 제목 더블클릭 최대화/복원을 사용하며 닫기는 기존 **닫기** 버튼으로만 수행합니다. 메시지함 부모를 닫으면 열린 DM 자식창과 그 안의 검색/프로필/설정 등 종속 팝업도 같이 닫힙니다. 작업영역이 기준보다 좁아지면 열린 자식창을 정리하고 현재 대화 하나를 단일-pane 화면으로 되돌립니다. 맵 어댑터 임베드 화면은 지원되는 멀티윈도우 배포 모드로 문서화하지 않습니다.
...
5.3.0의 DM에는 메시지 단위 **나에게만 숨김**이 없습니다. 사용자는 자신이 보낸 DM만 삭제할 수 있고, 삭제하면 양쪽 대화에서 제거되며 해당 메시지를 가리키던 Reply snapshot도 정리됩니다. `direct-message.confirm-delete`는 삭제 확인창 표시 여부를 제어합니다. 받은 DM은 수신자가 삭제할 수 없습니다. 서버 간 삭제는 대상 peer의 성공 응답을 받은 뒤 원본 서버에서도 제거합니다.
...
그룹 채팅도 **Standalone 데스크톱 모드**의 900 × 480 이상 작업영역에서는 방 목록을 부모 창으로 두고 각 그룹방을 별도 자식 창으로 열 수 있습니다. 이동/투명 8방향 resize/제목 더블클릭 최대화 규칙은 DM과 같고, 그룹 목록 부모를 닫으면 모든 그룹 자식창과 방 만들기·방 설정·멤버 관리·고정 메시지 등 해당 창에 종속된 팝업이 함께 닫힙니다. 좁은 화면에서는 멀티윈도우를 자동으로 사용하지 않습니다.
...
- 방장 이전
- 안 읽음 수 추적
- 방 내부 `owner/admin/member` 역할과 역할 변경
- 방별 고정 메시지 보기·고정·순서 변경·고정 해제
- 방 전체 메시지 삭제와 일반 멤버 자기 메시지 삭제 정책
...
- 오픈채팅과 그룹채팅의 **고정한 사용자** 표시는 일반 사용자 이름과 동일하게 표시명 / 실제 이름 전환을 따릅니다. 구조화된 고정자 정보가 없던 이전 오픈채팅 고정 기록은 기존 저장 이름을 fallback으로 사용합니다.
...
**Standalone 데스크톱 멀티윈도우**에서는 분리된 개별 DM/그룹 대화창도 각각 독립적인 드래그 앤 드롭 업로드 대상입니다. 파일을 자식 창에 놓으면 해당 DM 스레드/그룹 방을 활성화한 뒤 그 대화의 입력창에 업로드 URL을 넣으며, 기존 DM/그룹 목록 부모 창으로 드롭하는 동작도 그대로 사용할 수 있습니다.
...
자세한 보안 기준은 `UPLOAD_SECURITY.md`를 참고하세요.
...
필요하면 재접속하여 리소스 팩을 갱신합니다. 상세 내용은 `IMAGEEMOJIS_BERO_1_9_0.md`를 참고하세요.

## 23. 브라우저 알림과 Web Push

로그인 사용자의 키워드/알림 종류 설정은 5.0.0부터 계정 데이터에 저장되어 다른 브라우저와 기기에서도 공유됩니다. Windows/모바일처럼 시각 설정이 다른 경우에는 사용자별 UI 프로필을 여러 개 저장할 수 있으며, 창 위치·크기·최소화 상태와 Web Push endpoint는 기기 로컬 상태로 유지됩니다. 프로필 최대 개수와 JSON import/export 허용 여부는 Web Admin에서 관리합니다. 같은 계정으로 로그인된 KWC 화면 중 하나라도 실제로 보이고 포커스된 상태이며 채팅창이 최소화되지 않았고, DM/그룹창이 열려 있으며 현재 스레드/방 ID가 새 메시지 대상과 정확히 일치할 때만 이미 확인 중인 대화로 판정합니다. 이 상태는 짧은 브라우저/탭 heartbeat로 서버에 전달되며, 보고 있는 PC 자체에 Web Push 구독이 없어도 유효합니다. 해당 대화를 보고 있는 활성 화면이 하나라도 있는 동안에는 같은 계정의 모든 연결된 KWC 탭에서 그 DM/그룹의 브라우저 알림과 브라우저 로컬 알림함 추가를 막고, 모든 Web Push subscription에서도 발송 전에 제외합니다. 다른 방을 보고 있거나 창을 닫았거나 페이지가 숨김/비활성 상태이거나 채팅이 최소화되어 있으면 보고 있지 않은 것으로 처리되어 브라우저 알림/알림함/Web Push가 정상적으로 다시 동작합니다.
...
사용자는 채팅 설정에서 허용된 알림 종류를 다시 켜고 끌 수 있습니다. **이모지 반응**은 체크박스 하나가 브라우저 알림과 백그라운드/모바일 Web Push 양쪽에 공통 적용되고, 각 브라우저/기기에서는 지원하는 전달 방식만 동작합니다. **@멘션**은 반드시 `@` 뒤에 등록된 실제이름 또는 표시이름이 이어져야 동작합니다. 각 `@` 위치에서는 일치하는 등록 이름 중 가장 긴 이름 하나를 우선하고, 그 표시이름을 완전히 동일하게 공유하는 계정이 여러 개면 모두 호출되므로 공통 표시이름을 팀 호출처럼 사용할 수도 있습니다. 로그인 사용자의 알림 종류 선택은 계정 단위로 동기화되며 게스트는 현재 브라우저에만 저장됩니다. 서버 설정이 `false`인 종류는 사용자가 활성화할 수 없습니다. **메시지 미리보기는 기본 사양**으로 항상 사용되며 사용자/관리자용 별도 체크 항목이 없습니다. 브라우저 로컬 알림함은 최근 알림 대상 이벤트를 보관하고, 알림 클릭은 가능한 경우 공개 메시지·댓글·DM 스레드·그룹방으로 이동합니다.
...
direct relay는 5.0.0 방식처럼 요청별로 동작합니다. `/relay/v2/message`가 HKDF-SHA256 방향별 key와 AES-256-GCM으로 암호화·인증한 payload를 독립적으로 전달합니다. `/relay/v2/handshake`는 상태를 저장하지 않는 진단용 identity/health probe일 뿐 direct routing을 제어하지 않습니다. direct HTTP는 경고와 함께 허용됩니다. forwarding은 같은 group 안에서 peer 단위로 판단하며, http:// peer는 그 peer를 통한 forwarding만 제외되고 다른 https:// peer는 계속 사용할 수 있습니다.
...
전체 protocol 및 운영 기준은 `docs/ko/SERVER_RELAY.md`를 참고하세요.
...
- 메시지 삭제와 공개채팅 고정/삭제 동작 표시
- 고정 메시지 관리
- 게스트 및 IP 뮤트
- 사용자별 채팅/업로드 제한 및 사용자 프로필 이미지 삭제
- 모더레이터별 위임 권한 체크박스
- 활성 세션 확인 및 폐기
- 커스텀 이모지 폴더·파일 관리
- 업로드와 저장 사용량 확인
...
`direct-message.admin-audit.enabled`는 기본 OFF 읽기 전용 DM 본문 감사 스위치입니다. `private-chat-super-admins`에 정확히 지정된 계정만 접근할 수 있으며 감사 화면에서는 전송, 답글, 삭제, 읽음 처리를 할 수 없습니다. 각 페이지 열람은 `admin.dm-audit-read`로 기록됩니다. `group-chat.admin-audit.enabled`는 별도의 읽기 전용 그룹 본문 감사 스위치입니다.
...
- `CONFIGURATION.md`: 설정별 상세 설명
- `SERVER_RELAY.md`: 릴레이 토폴로지, 인증과 오류
- `UPGRADE.md`: 4.7.0→5.0.0 코어 분리/reload 안전 업그레이드
- `UPGRADE.md`: 4.6.3→4.7.0 기능 업그레이드
- `UPGRADE.md`: 4.6.2→4.6.3 업그레이드
- `UPGRADE.md`: 4.6.1→4.6.2 업그레이드
- `UPGRADE.md`: 4.6.0→4.6.1 업그레이드
- `UPGRADE.md`: 4.5.5→4.6.0 업그레이드
- `CADDY_HTTPS.md`: Caddy HTTPS 구성
- `NGINX_HTTPS.md`: Nginx HTTPS 구성
- `IMAGEEMOJIS_BERO_1_9_0.md`: ImageEmojis-Bero 연동
- `INSTALL_TROUBLESHOOTING.md`: 설치 문제 해결
- `UPLOAD_SECURITY.md`: 업로드 보안
- `OPERATIONS_SECURITY.md`: 공개 운영 보안
- `I18N.md`: 다국어 파일 관리
- `RELEASE_CHECKLIST.md`: 배포 전 검사

### 관리자 그룹채팅 본문 감사 (4.6.3)

`group-chat.admin-audit.enabled: true`를 설정하고 정확한 마인크래프트 이름 또는 UUID를 `private-chat-super-admins`에 등록해야 하며 두 조건이 모두 필요합니다. 조건을 만족한 관리자는 해당 방의 멤버가 아니어도 관리자 그룹 메타데이터 목록에서 본문을 읽기 전용으로 열 수 있습니다. 감사 화면은 방에 참여하지 않고 읽음/미확인 수를 변경하지 않으며 메시지 전송·업로드·삭제·멤버 변경도 제공하지 않습니다. 각 페이지 열람은 `admin.group-audit-read`로 기록되고 메시지 본문은 감사 로그에 복사하지 않습니다.



## SimpleNicks-Bero 연동

Bukkit/Paper 계열에서는 `player-display.mode: "display-name"`으로 [SimpleNicks-Bero](https://github.com/KOKOTO-DEV/SimpleNicks-Bero)가 Bukkit display name에 적용한 닉네임을 표시할 수 있습니다. 실제 연결 username/UUID는 KWC identity로 별도 유지됩니다. `SIMPLENICKS_BERO.md`를 참고하고, 일반 설치·운영은 [원본 SimpleNicks](https://github.com/Simplexity-Development/SimpleNicks)를 따르세요.
