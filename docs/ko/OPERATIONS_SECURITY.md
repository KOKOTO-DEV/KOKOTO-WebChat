...
KWC는 전달된 클라이언트 주소를 설정된 신뢰 프록시 체인에서 **오른쪽부터 왼쪽으로** 확인하고 첫 비신뢰 홉에서 멈춥니다. 따라서 클라이언트가 임의로 넣은 맨 왼쪽 `X-Forwarded-For` 값이 실제 클라이언트 주소를 덮어쓸 수 없습니다. 번들 단일 엣지 Nginx 예제도 기존 헤더에 이어 붙이지 않고 `$remote_addr`로 `X-Forwarded-For`를 덮어씁니다.
