# KOKOTO WebChat 5.3.0 릴리스 체크리스트

## 소스/설정/다국어
- [ ] Root/Bukkit/Fabric/NeoForge/Forge 메타데이터와 산출물 버전이 모두 `5.3.0`이다.
...
- [ ] Chat Event는 선착순/추첨을 여러 개 동시에 유지하고, 선착순은 당첨 인원 정원이 차면 자동 완료되며, 참가자/당첨자 목록은 표시이름 / 실제이름 전환을 따르고 결과 공지는 `🏆`로 시작해 이름이 다르면 `표시이름 (실제이름)`으로 표시한다.
