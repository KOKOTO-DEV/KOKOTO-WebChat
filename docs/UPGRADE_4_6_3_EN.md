# Upgrade to BlueMapWebChat 4.6.3
...
4.6.3 refreshes the bundled `config.yml` comments so they describe the current update checker, cross-server DM relay/read acknowledgements, group read status, and both administrator audit switches. On startup or `/bmchat reload`, an existing config may have comment text refreshed **only when that comment block still exactly matches an older bundled BlueMapWebChat comment**. Setting values are never changed by this comment refresh, custom comments are preserved, and `config-version` is still changed only by the administrator after reviewing the migration fragment.
