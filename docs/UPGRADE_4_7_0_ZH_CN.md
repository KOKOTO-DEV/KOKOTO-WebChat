...
只有由已配置的 `newline` / `blank-line` 标记生成的换行会以受保护状态通过 Minecraft 现有的单行清理，并在最终发送时作为独立的游戏聊天行输出。普通 CR/LF 输入仍按原有方式展平。通过服务器中继在游戏中显示这种有意换行时，接收端 BlueMapWebChat 也必须使用相同的 4.7.0 标记行发送支持；旧版接收端会在原有展平阶段把普通 LF 转为空格。
