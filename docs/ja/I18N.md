...
`ui.language` は、KOKOTO WebChat が `config.yml` のコメント/レイアウトを再構築し、`config-reference-5.3.0.yml` と migration/Difference の案内文を書き出す際の表示言語にも使用されます。内蔵の設定表示テンプレートは `en-US`、`ko-KR`、`ja-JP`、`zh-CN` です。既存の解析済み運用値を選択したテンプレートへ上書きするため、この4言語を切り替えてもコメント/レイアウトだけが変わり、実際の設定値は保持されます。
