...
latest-message auto-follow は公開/DM/group 共通で現在実際に描画される line-height を使います。最下部までの残り間隔がテキスト **2 行未満**の場合だけ最新メッセージを追従し、それより上を閲覧中なら新着メッセージや layout change だけで最下部へ引き戻しません。emoji/icon/attachment panel の開閉でも current viewport を保持し、更新/再接続後も可能な場合は保存済みの元の閲覧位置を復元します。
