...
KWC は転送されたクライアントアドレスを、設定された信頼プロキシチェーンの **右から左へ** 検証し、最初の非信頼ホップで停止します。これにより、クライアントが任意に付与した左端の `X-Forwarded-For` 値で実際のクライアントアドレスを上書きできません。付属の単一エッジ Nginx 例でも、受信値へ追記せず `$remote_addr` で `X-Forwarded-For` を上書きします。
