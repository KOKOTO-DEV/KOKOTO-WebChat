 ...
 * LangManager는 여러 저수준 객체를 조합해 하나의 KWC 기능 흐름을 수행하는 서비스/관리 계층이다.
 * LangManager is a service/manager layer coordinating lower-level objects into one KWC feature flow.
 *
...
public class LangManager implements WebChatLanguage {
    ...
    public LangManager(KokotoWebChatPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private void updateBuiltinLanguageFile(File target, String resource) {
        try (InputStream in = plugin.getResource(resource)) {
            ...
            plugin.getLogger().warning("Failed to update built-in language keys in " + target + ": " + ex.getMessage());
        }
    }



    private boolean migrateChangedBuiltinStrings(String language, YamlConfiguration existing, YamlConfiguration builtin) {
        ...
        for (Map.Entry<String,String> entry : previous.entrySet()) {
            ...
            // Only replace a value that is exactly one of KWC's previous bundled
            // strings. Administrator-customized translations remain untouched.
    ...
    private boolean migrateLegacyBranding(YamlConfiguration existing, YamlConfiguration builtin) {
        boolean changed = false;
        String[][] legacyValues = {
