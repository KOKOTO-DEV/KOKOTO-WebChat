 ...
 * ForgeLangManager는 여러 저수준 객체를 조합해 하나의 KWC 기능 흐름을 수행하는 서비스/관리 계층이다.
 * ForgeLangManager is a service/manager layer coordinating lower-level objects into one KWC feature flow.
 *
...
public class ForgeLangManager implements WebChatLanguage {
    ...
    public ForgeLangManager(KwcForgeRuntime plugin) {
        this.plugin = plugin;
    }
    ...
    private void updateBuiltinLanguageFile(File target, String resource) {
        try (InputStream in = plugin.resource(resource)) {
            ...
            ForgeYamlConfiguration existing = ForgeYamlConfiguration.loadConfiguration(target);
            ForgeYamlConfiguration builtin = ForgeYamlConfiguration.loadConfiguration(new java.io.InputStreamReader(in, java.nio.charset.StandardCharsets.UTF_8));
    ...
    private boolean migrateChangedBuiltinStrings(String language, ForgeYamlConfiguration existing, ForgeYamlConfiguration builtin) {
    ...
    private boolean migrateLegacyBranding(ForgeYamlConfiguration existing, ForgeYamlConfiguration builtin) {
        boolean changed = false;
        String[][] legacyValues = {
    ...
    private void loadInto(Map<String, String> target, String lang) {
        ...
        ForgeYamlConfiguration yml = ForgeYamlConfiguration.loadConfiguration(file);
        for (String key : defaultKeys().keySet()) {
    ...
    private Map<String, String> defaultKeys() {
        ...
        try (InputStream in = plugin.resource("lang/en-US.yml")) {
            if (in == null) return m;
            ForgeYamlConfiguration builtin = ForgeYamlConfiguration.loadConfiguration(new java.io.InputStreamReader(in, java.nio.charset.StandardCharsets.UTF_8));
            ForgeYamlSection web = builtin.getConfigurationSection("web");
