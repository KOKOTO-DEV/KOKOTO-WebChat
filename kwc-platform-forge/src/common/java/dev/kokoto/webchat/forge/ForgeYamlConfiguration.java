 ...
 * ForgeYamlConfiguration는 Forge 런타임에서 KWC core 기능을 해당 loader/Minecraft API에 연결한다.
 * ForgeYamlConfiguration connects KWC core behavior to the concrete Forge/Minecraft runtime APIs.
 *
...
/** Small SnakeYAML-backed configuration used by the Forge platform module. */
public final class ForgeYamlConfiguration extends ForgeYamlSection {
    public ForgeYamlConfiguration() {
        this(new LinkedHashMap<>());
    }

    private ForgeYamlConfiguration(Map<String,Object> root) {
        super(root, root, "");
    }

    public static ForgeYamlConfiguration empty() {
        return new ForgeYamlConfiguration(new LinkedHashMap<>());
    }

    public static ForgeYamlConfiguration loadConfiguration(File file) {
    ...
    public static ForgeYamlConfiguration loadStrict(File file) throws IOException {
        ...
        try (Reader reader = Files.newBufferedReader(file.toPath(), StandardCharsets.UTF_8)) {
            ...
            ForgeYamlConfiguration config = new ForgeYamlConfiguration(normalizeMap(map));
    ...
    public static ForgeYamlConfiguration loadConfiguration(Reader reader) {
        if (reader == null) return empty();
        try {
            Object loaded = new Yaml().load(reader);
            if (loaded instanceof Map<?, ?> map) {
                return new ForgeYamlConfiguration(normalizeMap((Map<?,?>) map));
            }
        } catch (Exception ignored) {
        }
        return empty();
    ...
    public Map<String,Object> flatLeafValues() {
        ...
        for (String path : getKeys(true)) {
            ForgeYamlSection child = getConfigurationSection(path);
            if (child != null) {
