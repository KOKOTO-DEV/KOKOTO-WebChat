...
public class ForgeYamlSection {
    protected final Map<String, Object> root;
    protected final Map<String, Object> section;
    protected final String sectionName;

    ForgeYamlSection(Map<String, Object> root, Map<String, Object> section, String sectionName) {
        this.root = root;
    ...
    @SuppressWarnings("unchecked")
    public ForgeYamlSection getConfigurationSection(String path) {
        Object v = get(path);
        if (!(v instanceof Map<?, ?> map)) return null;
        String name = path;
        int dot = name.lastIndexOf('.');
        if (dot >= 0) name = name.substring(dot + 1);
        return new ForgeYamlSection(root, (Map<String,Object>) map, name);
    }
