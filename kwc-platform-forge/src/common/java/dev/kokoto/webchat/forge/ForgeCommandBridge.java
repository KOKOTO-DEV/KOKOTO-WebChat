...
final class ForgeCommandBridge {
 ...
 void register(com.mojang.brigadier.CommandDispatcher<CommandSourceStack> dispatcher){
   ...
   var node=dispatcher.register(root); dispatcher.register(Commands.literal("kc").redirect(node));
 }
 ...
 private int reload(CommandSourceStack s){ if(!allowed(s,"kwc.admin")) return fail(s,"You do not have permission."); runtime.audit("command.reload",s.getTextName(),Map.of("platform","forge")); return runtime.reload()?ok(s,"KOKOTO WebChat reloaded."):fail(s,"KOKOTO WebChat configuration reload failed. Check the server log."); }
 private int status(CommandSourceStack s){var c=runtime.configValues(); if(c==null)return fail(s,"KOKOTO WebChat has not loaded a configuration yet."); String standalone=c.standaloneWebEnabled?(c.standaloneWebPath==null?"/":c.standaloneWebPath):"disabled"; return ok(s,"KOKOTO WebChat 5.0.0 | enabled="+c.pluginEnabled+" | HTTP="+c.httpHost+":"+c.httpPort+" | standalone="+standalone);}
