 ...
 * ForgeModerationManager는 메시지 moderation/content-filter 파이프라인의 일부다.
 * ForgeModerationManager is part of the message moderation/content-filter pipeline.
 *
...
public class ForgeModerationManager implements WebChatModeration {
    private final KwcForgeRuntime plugin;
    private final File file;
    private final Map<String, ModerationEntry> entries = new ConcurrentHashMap<>();

    public ForgeModerationManager(KwcForgeRuntime plugin) {
        this.plugin = plugin;
    ...
    public synchronized void load() {
        ...
        ForgeYamlSection root = yml.getConfigurationSection("entries");
        if (root == null) return;
        for (String key : root.getKeys(false)) {
            ForgeYamlSection s = root.getConfigurationSection(key);
            if (s == null) continue;
