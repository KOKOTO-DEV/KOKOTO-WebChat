 ...
 * ForgeStorage는 Forge 런타임에서 KWC core 기능을 해당 loader/Minecraft API에 연결한다.
 * ForgeStorage connects KWC core behavior to the concrete Forge/Minecraft runtime APIs.
 *
...
public class ForgeStorage implements WebChatStorage {
    private final KwcForgeRuntime plugin;
    ...
    public ForgeStorage(KwcForgeRuntime plugin) {
        this.plugin = plugin;
    ...
    public synchronized void load() {
        ...
        ForgeYamlConfiguration accounts = ForgeYamlConfiguration.loadConfiguration(accountsFile);
        ForgeYamlSection root = accounts.getConfigurationSection("accounts");
        if (root != null) {
            for (String id : root.getKeys(false)) {
                ForgeYamlSection s = root.getConfigurationSection(id);
                if (s == null) continue;
        ...
        ForgeYamlConfiguration sessions = ForgeYamlConfiguration.loadConfiguration(sessionsFile);
        ForgeYamlSection sr = sessions.getConfigurationSection("sessions");
        if (sr != null) {
            for (String hash : sr.getKeys(false)) {
                ForgeYamlSection s = sr.getConfigurationSection(hash);
                if (s == null) continue;
    ...
    public synchronized void saveAccounts() {
        ForgeYamlConfiguration yml = new ForgeYamlConfiguration();
    ...
    private synchronized void loadKnownDisplayNames() {
        knownDisplayNamesByUuid.clear();
        knownUsernamesByUuid.clear();
        ForgeYamlConfiguration yml = ForgeYamlConfiguration.loadConfiguration(knownDisplayNamesFile);
        ForgeYamlSection root = yml.getConfigurationSection("players");
        if (root == null) return;
        for (String uuid : root.getKeys(false)) {
            ForgeYamlSection s = root.getConfigurationSection(uuid);
            if (s == null) continue;
    ...
    public synchronized void loadPinnedMessages() {
        pinnedById.clear();
        ForgeYamlConfiguration yml = ForgeYamlConfiguration.loadConfiguration(pinnedFile);
        ForgeYamlSection root = yml.getConfigurationSection("pinned");
        if (root == null) return;
        for (String id : root.getKeys(false)) {
            ForgeYamlSection s = root.getConfigurationSection(id);
    ...
    private static PinnedMessage pinnedMessageFromSection(ForgeYamlSection s) {
