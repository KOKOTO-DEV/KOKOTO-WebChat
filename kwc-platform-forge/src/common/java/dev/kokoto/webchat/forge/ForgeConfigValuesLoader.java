 ...
 * ForgeConfigValuesLoader는 KWC 설정을 core가 사용할 수 있는 형태로 읽거나 보관하는 설정 계층이다.
 * ForgeConfigValuesLoader is part of the configuration layer that reads or carries KWC settings in a core-friendly form.
 *
...
/** Forge YAML loader for the platform-neutral ConfigValues data object. */
public final class ForgeConfigValuesLoader {
    private ForgeConfigValuesLoader() {}

    public static ConfigValues load(ForgeYamlConfiguration c) {
        ...
        ForgeYamlSection tokenCustom = c.getConfigurationSection("message-tokens.custom");
        if (tokenCustom != null) {
            for (String key : tokenCustom.getKeys(false)) {
                ForgeYamlSection item = tokenCustom.getConfigurationSection(key);
        ...
        ForgeYamlSection gameLinkAliases = c.getConfigurationSection("emoji.game-link.aliases");
    ...
    private static void loadAnnouncement(ConfigValues v, ForgeYamlConfiguration c, String key, boolean defaultEnabled, String defaultMessage) {
    ...
    private static boolean configBool(ForgeYamlConfiguration c, String path, boolean def) {
        return c != null && c.contains(path) ? c.getBoolean(path) : def;
    }

    private static boolean notificationBool(ForgeYamlConfiguration c, String key, boolean def) {
        return configBool(c, "notifications." + key, def);
    }

    private static boolean legacyPairBool(ForgeYamlConfiguration c, String browserPath, String webPushPath, boolean def) {
