...
public final class ForgeCompat {
 private ForgeCompat(){}
 public static String profileName(ServerPlayer p){
  if(p==null)return "";
  try{
   Object profile=p.getGameProfile();
   Object name=invokeNoArg(profile,"name","getName");
   if(name!=null)return String.valueOf(name);
  }catch(Throwable ignored){}
  return p.getName().getString();
 }
 public static boolean isOperator(MinecraftServer s, ServerPlayer p){
  if(s==null||p==null)return false;
  try{
   Object identity=invokeNoArg(p,"nameAndId","getGameProfile");
   if(identity==null)return false;
   for(Method m:s.getPlayerList().getClass().getMethods()){
    if(!m.getName().equals("isOp")||m.getParameterCount()!=1)continue;
    Class<?> type=m.getParameterTypes()[0];
    if(!type.isAssignableFrom(identity.getClass()))continue;
    Object result=m.invoke(s.getPlayerList(),identity);
