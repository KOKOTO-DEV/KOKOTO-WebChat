 ...
 * ForgeGameMessageRenderer는 Minecraft/loader 버전 차이를 흡수하는 Forge compatibility shim이다.
 * ForgeGameMessageRenderer is a Forge compatibility shim absorbing Minecraft/loader API differences across target versions.
 *
...
/** Minecraft 1.18.2 native chat-component renderer used by the Forge platform. */
final class ForgeGameMessageRenderer {
    private static final Pattern URL_PATTERN = Pattern.compile("(?i)((?:https?://|www\\.)[^\\s<>\"]+)");

    private ForgeGameMessageRenderer() {}
    ...
    private static void appendOpenUrl(MutableComponent root, String url) {
        ...
        try {
            event = new ClickEvent(ClickEvent.Action.OPEN_URL, normalizeClickUrl(url));
    ...
    private static void appendLegacyEvent(MutableComponent root, String text, String hoverText, ClickEvent clickEvent) {
        ...
        HoverEvent hover = hoverText == null || hoverText.isBlank()
                ? null : new HoverEvent(HoverEvent.Action.SHOW_TEXT, plain(hoverText));
    ...
    private static void flush(MutableComponent root, StringBuilder segment, Style style) {
        ...
        root.append(new TextComponent(segment.toString()).setStyle(style));
        segment.setLength(0);
    }
