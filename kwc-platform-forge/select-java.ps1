...
function Invoke-KwcJsonRequest([string]$Uri) {
    $headers = @{ 'User-Agent' = 'KOKOTO-WebChat-Build/5.3.1' }
    $lastError = $null
    ...
    if ($curl) {
        ...
        try {
            & $curl.Source '--fail' '--silent' '--show-error' '--location' '--retry' '3' '--retry-delay' '2' '--connect-timeout' '20' '--max-time' '90' '--user-agent' 'KOKOTO-WebChat-Build/5.3.1' '--output' $tmp $Uri
...
function Invoke-KwcDownload([string]$Uri, [string]$OutFile) {
    ...
    if ($curl) {
        ...
        if (Test-Path -LiteralPath $OutFile) { Remove-Item -LiteralPath $OutFile -Force -ErrorAction SilentlyContinue }
        & $curl.Source '--fail' '--silent' '--show-error' '--location' '--retry' '3' '--retry-delay' '2' '--connect-timeout' '20' '--user-agent' 'KOKOTO-WebChat-Build/5.3.1' '--output' $OutFile $Uri
