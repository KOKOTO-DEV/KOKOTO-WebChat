...
`build-target.bat`/`build-all.bat` first auto-detect common Windows JDK installations, `JAVA_HOME`, and JDKs exposed through `javac.exe` on `PATH`. If the required major is missing, the Windows resolver downloads the latest GA Eclipse Temurin JDK ZIP from the official Adoptium API into `kwc-platform-forge/.jdks/`, verifies the published SHA-256 checksum, extracts it locally, and uses that copy without changing the system Java installation or persistent `JAVA_HOME`. Network lookup/download is retried and falls back to Windows `curl.exe` when the Windows PowerShell web stack fails. You can override selection with `KWC_JAVA17_HOME`, `KWC_JAVA21_HOME`, and `KWC_JAVA25_HOME`. Set `KWC_AUTO_DOWNLOAD_JDK=0` only if you intentionally want missing JDKs to fail instead of being downloaded.
